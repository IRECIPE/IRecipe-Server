...
public interface IngredientRepository extends JpaRepository<Ingredient, Long> {
    ...
    @Query("SELECT i.name FROM Ingredient i JOIN i.member m WHERE m.personalId = :memberId")
    List<String> findNamesByMemberId(@Param("memberId") String memberId);

    // 유통기한으로 정렬된 사용자의 재료 리스트 조회
    @Query("SELECT i.name FROM Ingredient i JOIN i.member m WHERE m.personalId = :memberId ORDER BY i.expiry_date ASC")
